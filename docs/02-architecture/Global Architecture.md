...
1. `gateway-core` validates runtime configuration and loads all allowed active deployments from PostgreSQL.
2. The gateway builds an environment-grouped registry of proxies, endpoints, and validated policy configuration.
3. Incoming hostnames select an environment before paths resolve in that registry.
...
- Unknown business hostnames return `421`; there is no default environment.

## Constraints

- `gateway-core` reads configuration but does not provide management CRUD.
- Control-plane reads include environments, proxies, and deployments; mutations
  are currently limited to apps, CAs, certificates, PKI status, and audit context.
