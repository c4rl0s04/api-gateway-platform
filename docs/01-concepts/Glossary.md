...
| **Endpoint**                | A route within a logical proxy, with a public path, backend targetPath and policies.                                                                 |
| **ProxyDeployment**         | Deployment of a logical proxy into one Environment. Contains the upstream server and activation state.                                               |
| **UpstreamBaseUrl**         | Backend origin configured per deployment and combined with each endpoint targetPath.                                                                 |
