...
DEVELOPER_TOKEN_SECRET_FILE="$SECRETS_DIR/oauth/developer-token-issuance.secret"
if [[ ! -f "$DEVELOPER_TOKEN_SECRET_FILE" ]]; then
  umask 077
  openssl rand -base64 48 > "$DEVELOPER_TOKEN_SECRET_FILE"
fi
DEVELOPER_TOKEN_ISSUANCE_SECRET="$(tr -d '\n' < "$DEVELOPER_TOKEN_SECRET_FILE")"
...
printf '%s\n' \
  ...
  "DEVELOPER_TOKEN_ISSUANCE_SECRET=$DEVELOPER_TOKEN_ISSUANCE_SECRET" \
